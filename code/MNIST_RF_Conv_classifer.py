...
def main():
	...
	'''Management Data'''
	data_path='../input/'
	model_folder='../model/RF_Conv/0/'
	model_Name='titanic.ckpt'
	'''Management Data'''
	'''Data prepare, rotate or shuffle'''
	TrainSet,ValSet,TestSet=data_preparer(data_path,0.5)
	clf = ensemble.RandomForestClassifier(n_estimators=200,min_samples_leaf=9,max_depth=100,max_features='auto',oob_score=True)
...
def conv(il,m_s,i_s):
	...
	for i in range(length):
		a=il.pop(0)
		a=signal.convolve2d(a,mask,boundary='symm',mode='same')
		il.append(a)
	il=np.array(il).reshape(length,(i_s)**2)
	return il
...
def data_preparer(path,train_perc):
	...
	for i in range(conv_layers):
		train_x=conv(train_x,mask_size,image_size)
		print(train_x.shape)

	# train_x = ss.fit_transform(train_x,label)
	...
	#for i in range(conv_layers):
	# 	test_x=conv(test_x,mask_size,image_size)
	# 	print(test_x.shape)
